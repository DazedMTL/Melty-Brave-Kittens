...
(function () {
  ...
  Game_Interpreter.prototype.execStartRotation = function (args) {
    $gameScreen.startRotation(
      getArgNumber(args[0]),
      args.length > 1 ? getArgNumber(args[1], 0) : undefined
    );
  };
  ...
  Game_Interpreter.prototype.execStartZoom = function (args) {
    ...
    var x =
      args.length > 2 ? getArgNumber(args[2]) : SceneManager._screenWidth / 2;
    var y =
      args.length > 3 ? getArgNumber(args[3]) : SceneManager._screenHeight / 2;
    if (duration > 0) {
  ...
  var _Game_CharacterBase_isTransparent =
    Game_CharacterBase.prototype.isTransparent;
  Game_CharacterBase.prototype.isTransparent = function () {
    return (
      _Game_CharacterBase_isTransparent.apply(this, arguments) ||
      this._hiddenOuterScreen
    );
  };
