...
TMPlugin.Shooting.HoldType =
  TMPlugin.Shooting.Parameters["holdType"] || "SWITCH";
TMPlugin.Shooting.DeadSwitch =
  TMPlugin.Shooting.Parameters["deadSwitch"] || "A";
TMPlugin.Shooting.ResetDeadSwitch =
  TMPlugin.Shooting.Parameters["resetDeadSwitch"] === "1";
TMPlugin.Shooting.BulletBlockTag = +(
  TMPlugin.Shooting.Parameters["bulletBlockTag"] || -1
);
TMPlugin.Shooting.BulletBlockRegion = +(
  TMPlugin.Shooting.Parameters["bulletBlockRegion"] || -1
);
TMPlugin.Shooting.DefaultDeadAnimeId = +(
  TMPlugin.Shooting.Parameters["defaultDeadAnimeId"] || 67
);
TMPlugin.Shooting.LevelUpAnimeId = +(
  TMPlugin.Shooting.Parameters["levelUpAnimeId"] || 52
);
TMPlugin.Shooting.LeaderShotSe = JSON.parse(
  TMPlugin.Shooting.Parameters["leaderShotSeParam"] || "{}"
);
TMPlugin.Shooting.LeaderShotSe.name =
  TMPlugin.Shooting.Parameters["leaderShotSe"] || "";
TMPlugin.Shooting.PlayerDeadEventId = +(
  TMPlugin.Shooting.Parameters["playerDeadEventId"] || 0
);
TMPlugin.Shooting.InvincibleFollower = JSON.parse(
  TMPlugin.Shooting.Parameters["invincibleFollower"]
);
TMPlugin.Shooting.UseGameover = JSON.parse(
  TMPlugin.Shooting.Parameters["useGameover"]
);
TMPlugin.Shooting.MaxPlayerBullet = +(
  TMPlugin.Shooting.Parameters["maxPlayerBullet"] || 128
);
TMPlugin.Shooting.MaxEnemyBullet = +(
  TMPlugin.Shooting.Parameters["maxEnemyBullet"] || 128
);
TMPlugin.Shooting.EquipDummyX = +(
  TMPlugin.Shooting.Parameters["equipDummyX"] || 408
);
TMPlugin.Shooting.EquipDummyY = +(
  TMPlugin.Shooting.Parameters["equipDummyY"] || 312
);
TMPlugin.Shooting.UseLevelUpMessage = JSON.parse(
  TMPlugin.Shooting.Parameters["useLevelUpMessage"]
);
...
TMPlugin.Shooting.Parameters["bulletSizeTable"]
  .split(",")
...
TMPlugin.Shooting.Parameters["bulletBlendTable"]
  .split(",")
...
(function () {
        ...
        if (members[i].actorId() === this.actorId()) {
          if (i === 0) {
            ...
            $gamePlayer
              .followers()
              .follower(i - 1)
              .requestAnimation(TMPlugin.Shooting.LevelUpAnimeId);
          }
        }
      }
    }
  ...
  var _Game_Actor_shouldDisplayLevelUp =
  ...
  Game_Actor.prototype.refreshShotParam = function () {
    ...
    if (weapon) {
      ...
      this._shotParams.skillId = +(
        weapon.meta.shotSkill || this.attackSkillId()
      );
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (item) {
        ...
        if (item.meta.shotSkill)
          this._shotParams.skillId = +item.meta.shotSkill;
      }
    }
  };
  ...
  if (Imported.SAN_MapGenerator) {
    // 非地面タイルを通路タイルに変換
    var _Game_MapGenerator_notGroundToPass =
      Game_MapGenerator.prototype.notGroundToPass;
  ...
  // 弾の追加
  Game_Map.prototype.addBullet = function (
    x,
    y,
    z,
    vx,
    ...
    if (ownerId < 0) {
      if (this._blankPlayerBullets.length > 0) {
        ...
        this._playerBullets[bulletIndex].setup(
          x,
          y,
          z,
          vx,
      ...
      if (this._blankEnemyBullets.length > 0) {
        ...
        this._enemyBullets[bulletIndex].setup(
          x,
          y,
          z,
          vx,
  ...
  Game_Bullet.prototype.angleToCharacter = function (character) {
    return Math.atan2(
      character._realY - character.collideH / 2 - this._y,
  ...
  Game_Bullet.prototype.executeDamage = function (character) {
    ...
    if (this._ownerId < 0) {
      var owner =
        this._ownerId === -1
          ? $gamePlayer
          : $gamePlayer.followers().follower(-this._ownerId - 2);
    } else {
      var owner =
        this._ownerId > 0 ? $gameMap.bulletEvent(this._ownerId) : null;
    }
  ...
  Game_Bullet.prototype.isCollidedWithPlayerCharacter = function () {
    ...
    return $gamePlayer
      .followers()
      .visibleFollowers()
      .some(function (follower) {
        if (this.isCollidedWithCharacter(follower)) {
          if (
            follower.battler().isAlive() &&
  ...
  Game_Character.prototype.angleToCharacter = function (character) {
    return Math.atan2(
      character._realY -
        character._collideH / 2 -
        (this._realY - this._collideH / 2),
  ...
  // ｎ方向ショット
  Game_Character.prototype.nwayShot = function (
    n,
    space,
    angle,
    speed,
    ...
    var y =
      this._realY +
      1 -
      (this.shiftY() - this._shiftFiringY) / $gameMap.tileHeight() -
      this._collideH / 2;
    for (var i = 0; i < n; i++) {
      $gameMap.addBullet(
        x,
  ...
  // 自機狙いｎ方向ショット
  Game_Character.prototype.nwayAim = function (
    n,
    space,
    angle,
    speed,
  ...
  // 全方位ショット
  Game_Character.prototype.nallShot = function (
    n,
    angle,
    speed,
    count,
  ...
  // 自機狙い全方位ショット
  Game_Character.prototype.nallAim = function (
    n,
    angle,
    speed,
    count,
  ...
  Game_Character.prototype.executeShot = function () {
    ...
    if (battler && battler.canMove()) {
      ...
      if (shotParams.way > 0) {
        var angle =
          this._shotAngle == null
            ? this.shotAngleFromDirection()
            : this._shotAngle;
        this._shotDelay = shotParams.interval;
        return this.nwayShot(
          shotParams.way,
          shotParams.space,
          angle,
          shotParams.speed,
  ...
  Game_Character.prototype.shotAngleFromDirection = function () {
    if (Imported.SAN_AnalogMove) {
      if (
        Sanshiro.AnalogMove &&
        Sanshiro.AnalogMove.version.slice(0, 1) === "3"
      ) {
  ...
  Game_Player.prototype.isDirectionFixed = function () {
    if (this.isMoveRouteForcing())
      return _Game_Player_isDirectionFixed.call(this);
    ...
    if (this._shotDelay > 0) {
      ...
      if (this.executeShot()) {
        ...
        if (shotParams.seName) {
          AudioManager.playSe({
            name: shotParams.seName,
            volume: shotParams.seVolume,
            pitch: shotParams.sePitch,
            pan: 0,
          });
  ...
  Sprite_EquipDummy.prototype.setActor = function (actor) {
    ...
    this._shiftFiringY =
      +(actor.actor().meta.shiftFiringY || 0) / $gameMap.tileHeight();
  };
  ...
  Sprite_EquipDummy.prototype.shotY = function () {
    return (
      this.y / $gameMap.tileHeight() - this._shotShiftY + this._shiftFiringY
    );
  };
  ...
  var _Sprite_Destination_updatePosition =
    Sprite_Destination.prototype.updatePosition;
  Sprite_Destination.prototype.updatePosition = function () {
    if (
      Imported.SAN_AnalogMove &&
      Sanshiro.SAN_AnalogMove &&
  ...
  Scene_Equip.prototype.nwayShot = function (
    n,
    space,
    angle,
    speed,
  ...
  Scene_Equip.prototype.addBullet = function (
    x,
    y,
    z,
    vx,
