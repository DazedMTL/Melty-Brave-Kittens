...
(function () {
  ...
  Game_Message.prototype.translateMessage = function () {
    ...
    TranslationManager.getTranslatePromise(this.getTranslateMessage()).then(
      this.setTranslationMessage.bind(this)
    );
  };
  ...
  Window_Message.prototype.updateLoading = function () {
    return (
      _Window_Message_updateLoading.apply(this, arguments) ||
      $gameMessage.isTranslating()
    );
  };
  ...
  //  翻訳の実行を管理します。
  //=============================================================================
  TranslationManager._accessTokenUrl =
    "https://api.cognitive.microsoft.com/sts/v1.0/issueToken";
  TranslationManager._translateUrlV3 =
    "https://api.cognitive.microsofttranslator.com/translate";
  TranslationManager._subscriptionKey1 =
    param.SubscriptionKey || "f78dab007fdb4ad4ad5baaaa01b74829";
  ...
  TranslationManager.update = function () {
    if (
      this._interval === 0 &&
      this._databaseTranslator &&
      !this._translateError
    ) {
  ...
  TranslationManager.translateProperty = function (propertyName) {
    ...
    var targetText =
      this._currentData[propertyName + "Original"] ||
      this._currentData[propertyName];
    this._continue = true;
    this.translateIfNeed(
      targetText,
      this.setTranslatedData.bind(this, propertyName)
    );
    return this._continue;
  };

  TranslationManager.setTranslatedData = function (
    propertyName,
    translatedText
  ) {
    var originalProperty = propertyName + "Original";
    if (!translatedText || this._currentData.hasOwnProperty(originalProperty))
      return;
  ...
  TranslationManager.translateIfNeed = function (targetText, callBack) {
    ...
    this.requestExecute(targetText).then(
      function (transResults) {
        ...
        callBack(
          this.isValidTranslation() && translatedText
            ? translatedText
            : targetText
        );
      }.bind(this),
  ...
  TranslationManager.addDictionaryData = function (
    targetText,
    translatedText,
    language
  ) {
  ...
  TranslationManager.requestExecute = function (text) {
    return this.requestAccessToken().then(
      this.requestTranslate.bind(this, text)
    );
  };
  ...
  TranslationManager.isValidTranslation = function () {
    ...
    var result = $gameSwitches
      ? $gameSwitches.value(param.TranslationSwitchId)
      : true;
  ...
  DataManager.loadTranslateData = function () {
    param.ToLanguage.forEach(function (language) {
      var client = new GameWebClient(
        this._languageFileUrl.format(language),
        "GET"
      );
      client
        .request()
        .then(
          this.onLoadLanguageData.bind(this, language),
  ...
  DataManager.saveTranslateData = function () {
    ...
    param.ToLanguage.forEach(function (language) {
      StorageManager.saveTranslateFile(
        this._languageFileUrl.format(language),
  ...
  DataManager.isDatabaseLoaded = function () {
    var result =
      _DataManager_isDatabaseLoaded.apply(this, arguments) && !!$dataTransDic;
  ...
  DataManager.isMapLoaded = function () {
    ...
    if (loaded && this._mapNameLoadingStatus === 1) {
      ...
      TranslationManager.getTranslatePromise($dataMap.displayName).then(
        this.onTranslateDisplayName.bind(this)
      );
    }
  ...
  class GameWebClient {
    constructor(url, method) {
      ...
      this._xmlHttpRequest.addEventListener(
        "readystatechange",
    ...
    getPromise() {
      return new Promise(
        function (resolve, reject) {
          this._xmlHttpRequest.addEventListener(
            "load",
            this.onLoad.bind(this, resolve, reject)
          );
          this._xmlHttpRequest.addEventListener(
            "error",
            this.onError.bind(this, reject)
          );
        }.bind(this)
      );
    }
    ...
    createQueryText() {
      var paramArray = this._queryParams.map(function (paramHash) {
        return (
          encodeURIComponent(paramHash.name) +
          "=" +
          encodeURIComponent(paramHash.value)
        );
      });
      ...
      if (keys && keys.length > 0) {
        keys.forEach(function (paramName) {
          this._xmlHttpRequest.setRequestHeader(
            paramName,
            this._requestHeaders[paramName]
          );
        }, this);
      }
    }
    ...
    getReadyStateMessage() {
      ...
      switch (xhr.readyState) {
        ...
        case 4:
          value =
            (this.isStatusOk() ? "Success" : "Failure") + "[Code]" + xhr.status;
          break;
      }
      return value;
    }
