...
(function () {
  ...
  (function () {
    ...
    MPPlugin.atbSpeed = Number(parameters["ATB Speed Default"] || 2).clamp(
      0,
      4
    );
    MPPlugin.atbSpeedName = parameters["ATB Speed Name"];
    MPPlugin.atbSpeedStatus = (
      parameters["ATB Speed Status"] || "1,2,3,4,5"
    ).split(",");

    //=== Battle ===
    MPPlugin.resetAtDie = !!eval(parameters["Reset AT Die?"]);
    MPPlugin.needEscapeAt = Number(parameters["Need Escape At"] || 100).clamp(
      0,
      100
    );
    MPPlugin.escapeAtCost = Number(parameters["Escape At Cost"] || 75).clamp(
      0,
      100
    );
    ...
    MPPlugin.atbFastRate = Number(parameters["ATB Fast Rate"] || 3).clamp(
      0,
      10
    );
    ...
    MPPlugin.ChangeModeInBattle = !!eval(
      parameters["Change Mode in Battle _v2"]
    );
    ...
    //=== Window ===
    MPPlugin.helpWindowPos = Number(parameters["Help Window Pos"] || 1).clamp(
      -1,
      1
    );
    MPPlugin.helpWindowRow = Number(parameters["Help Window Row"] || 1).clamp(
      0,
      2
    );
    MPPlugin.stWindowPos = Number(parameters["Status Window Pos"] || 1).clamp(
      0,
      2
    );
    ...
    MPPlugin.atChargeColor1 = "rgb(%1)".format(
      parameters["AT Charge Color1"] || "192,192,192"
    );
    MPPlugin.atChargeColor2 = "rgb(%1)".format(
      parameters["AT Charge Color2"] || "255,255,255"
    );
    MPPlugin.atMaxColor1 = "rgb(%1)".format(
      parameters["AT Max Color1"] || "192,192,192"
    );
    MPPlugin.atMaxColor2 = "rgb(%1)".format(
      parameters["AT Max Color2"] || "255,255,192"
    );
    MPPlugin.chantingView = !!eval(parameters["Chanting View?"]);
    MPPlugin.atChantingColor1 = "rgb(%1)".format(
      parameters["AT Chanting Color1"] || "128,32,0"
    );
    MPPlugin.atChantingColor2 = "rgb(%1)".format(
      parameters["AT Chanting Color2"] || "255,64,0"
    );
    MPPlugin.EscapingChange = !!eval(parameters["Escaping Change?"]);
    MPPlugin.atEscapingColor1 = "rgb(%1)".format(
      parameters["AT Escaping Color1"] || "192,192,192"
    );
    MPPlugin.atEscapingColor2 = "rgb(%1)".format(
      parameters["AT Escaping Color2"] || "192,192,255"
    );
  })();
  ...
  ConfigManager.atbMode = Math.max(
    MPPlugin.atbModeStatus.indexOf(MPPlugin.atbMode),
    0
  );
  ...
  if (!ConfigManager.MppOption3) {
    ...
    Window_Options.prototype.getMppStatus3 = function (symbol) {
      var params = ConfigManager.mppParams3;
      for (var i = 0; i < params.length; i++) {
        if (params[i].symbol === symbol) return params[i].status;
      }
      return [];
    };
  ...
  BattleManager.setup = function (troopId, canEscape, canLose) {
    ...
    var rate =
      (7 - ConfigManager.getMppParam("atbSpeed")) * MPPlugin.atbSpeedBase * 16;
  ...
  BattleManager.updateCmdActor = function () {
    if (this.isEvantWait() || this.isBattleEnd()) return this.clearActor();
  ...
  Game_Battler.prototype.atGaugeColorEx1 = function () {
    if (MPPlugin.chantingView && this.castRate() >= 0) {
      ...
      MPPlugin.EscapingChange &&
      this.isActor() &&
      BattleManager.isEscaping() &&
      this.isAlive()
    ) {
  ...
  //80
  Alias.WiBaSt_drawGaugeAreaWithTp =
  ...
  //86
  Alias.WiBaSt_drawGaugeAreaWithoutTp =
  ...
  Window_BattleStatus.prototype.drawActorAt = function (
    actor,
    x,
    y,
    width,
  ...
  Window_BattleStatus.prototype.drawAtGauge = function (
    x,
    y,
    width,
    rate,
  ...
  Scene_Battle.prototype.isAtbWait = function () {
    ...
    if (mode === "wait") {
      if (
        ...
        this._itemWindow.visible ||
        this._actorWindow.visible ||
      ...
      if (
        !!this._actor &&
        (!MPPlugin.ModeStopFastEneble || !BattleManager.isFastForward())
      ) {
        return true;
      }
    }
  ...
  Scene_Battle.prototype.changeInputWindow = function () {
    if (
      !this._partyCommandWindow.isClosed() ||
      this._actorCommandWindow.isClosing()
    ) {
      return;
    } else if (this._actor !== BattleManager.actor()) {
      if (MPPlugin.FastCancelByInput) BattleManager._atbFast = false;
  ...
  Scene_Battle.prototype.createStatusWindow = function () {
    ...
    this._statusWindow.setHandler(
      "pagedown",
      this.selectNextCommand.bind(this)
    );
  };

  //166
  Alias.ScBa_createPartyCommandWindow =
  ...
  //242
  Alias.ScBa_startPartyCommandSelection =
  ...
  //259
  Alias.ScBa_startActorCommandSelection =
