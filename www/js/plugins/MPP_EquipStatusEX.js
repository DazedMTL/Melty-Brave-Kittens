...
(function () {
...

  {
    ...
    MPPlugin.GaugeColor = "rgb(%1)".format(
      parameters["Gauge Color"] || "224,255,255"
    );
  ...
  Window_EquipStatus.prototype.createEquipParamsList = function (maxRow) {
    ...
    if (maxRow > list.length) {
      ...
      status = status.filter(
        (id) => !list.contains(id) && this.includeParam(id)
      );
    ...
    if (maxRow > list.length) {
      ...
      status = status.filter(
        (id) => !list.contains(id) && this.isChangedParam(id)
      );
      status = status.slice(0, maxRow - list.length);
      list = list.concat(status);
    }
  ...
  Window_EquipStatus.prototype.createStateRateList = function (maxRow) {
    ...
    if (MPPlugin.terms.stateRate && type > 0) {
      ...
      states = states.filter((id) =>
        this.includeRate(type, code, "stateRate", id)
      );
  ...
  Window_EquipStatus.prototype.createOriginalTraitList = function (maxRow) {
    ...
    if (maxRow > list.length && (type === 3 || type === 4)) {
      ...
      list = list.concat(
        curTraits.filter((t) => !list.contains(t) && !newTraits.contains(t))
      );
      list = list.concat(
        newTraits.filter((t) => !list.contains(t) && !curTraits.contains(t))
      );
    }
  ...
  Window_EquipStatus.prototype.drawParamGauge = function (
    x,
    y,
    width,
    curValue,
    ...
    var value =
      newValue - ((newValue - curValue) * Math.pow(count, 2)) / Math.pow(24, 2);
  ...
  Window_EquipStatus.prototype.drawFlatGauge = function (
    x,
    y,
    curWidth,
    newWidth,
    ...
    if (curWidth !== newWidth) {
      if (curWidth > newWidth) this.contents.paintOpacity = 192;
  ...
  Window_EquipStatus.prototype.drawArcGauge = function (
    x,
    y,
    curWidth,
    newWidth,
    ...
    if (curWidth !== newWidth) {
      if (curWidth > newWidth) this.contents.paintOpacity = 160;
  ...
  Window_EquipStatus.prototype.draw2LineGauge = function (
    x,
    y,
    curWidth,
    newWidth,
  ...
  Window_EquipStatus.prototype.drawNewParam = function (
    x,
    y,
    paramId,
    reverse
  ...
  Window_EquipStatus.prototype.includeRate = function (type, code, method, id) {
    switch (type) {
      case 1:
        return true;
      case 2:
        return this.includeTrait(code, id);
      case 3:
        return this.isChangedRate(method, id);
      case 4:
        return this.includeTrait(code, id) || this.isChangedRate(method, id);
      default:
        return false;
    }
  };
  ...
  if (!Window_Base.Mpp_UpdateDrawer) {
    ...
    Window_Base.prototype.updateDrawer = function () {
      if (this.isOpen() && this.visible && this._updateDrawers.length > 0) {
        this._updateDrawers = this._updateDrawers.filter((process) =>
          process()
        );
      }
    };
