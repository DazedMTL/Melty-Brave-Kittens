...
(function () {
  ...
  param.validUpPlayer = getParamBoolean([
    "ValidUpPlayer",
    "プレイヤー上向き許容",
  ]);
  ...
  if (!param.imageOnly) {
    ...
    Game_CharacterBase.prototype.modifyDirectionForHorizontalMove = function (
      prevDirection
    ) {
      if (
        this.isNeedModifyDirection() &&
        !this.isOnLadder() &&
        !this.isDirectionFixed()
      ) {
  ...
  Game_CharacterBase.prototype.isNeedModifyDirection = function () {
    return (
      this.direction() === 2 ||
      (this.isNeedModifyUpper() && this.direction() === 8)
    );
  };
  ...
  if (param.imageOnly) {
    ...
    Sprite_Character.prototype.characterPatternY = function () {
      ...
      if (
        this._character.isHorizontalMove() &&
        this._character.isNeedModifyDirection() &&
        this._prevPatternY
      ) {
