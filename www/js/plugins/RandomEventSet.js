...
(function () {
  ...
  var regionSetKey = RegExp(
    parameters["RegionSetKey"] || "<ランダムリージョン:(\\d+)>"
  );

  var _rSet_GInterpreter_pluginCommand =
  ...
  RandomSetManager.randomSettingAtAll = function () {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      if (e.event().note.match(randomSetKey)) {
        ...
        $gameSystem._regionArray[$gameMap.mapId()][e.eventId()] = [
          e.eventId(),
          0,
          x,
          y,
  ...
  RandomSetManager.isCheckExistRegion = function (regionId) {
    if (this._existRegion === undefined) {
      this._existRegion = [];
    }
    if (this._existRegion.contains(regionId)) {
      return true;
    }
  ...
  RandomSetManager.initCheckRegionArray = function () {
    if ($gameSystem._regionArray === undefined) {
      $gameSystem._regionArray = {};
    }
  ...
  Game_Player.prototype.performTransfer = function () {
    if ($gameSwitches.value(initTransfer)) {
      $gameSystem.clearRegionArray();
    }
    if ($gameMap.mapId() === this._newMapId && this.isTransferring()) {
      if ($gameSwitches.value(randomSetSwitchId)) {
        RandomSetManager.randomSettingAtAll();
      }
      if ($gameSwitches.value(regionSetSwitchId)) {
        RandomSetManager.randomSettingAtRegion();
      }
  ...
  Game_System.prototype.initRegionArray = function () {
    if (this._regionArray === undefined) {
      this._regionArray = {};
    }
  };
  ...
  Game_System.prototype.setInitRegionArray = function (mapId) {
    if (this._regionArray === undefined) {
      this.initRegionArray();
    }
    if (this._regionArray[mapId] === undefined) {
      this._regionArray[mapId] = {};
    }
  };
