...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (command.toUpperCase() === "HZTIMINGBAR") {
      ...
      var y =
        args[5] != null ? Number(args[5]) : SceneManager._screenHeight / 2;

      var hitArea = hitAreaParm.split("-").map(function (elm) {
        return Number(elm);
      });
      var criticalArea = criticalAreaParm.split("-").map(function (elm) {
        return Number(elm);
      });
      var requiredAreas = requiredAreaParm
        .substring(1, requiredAreaParm.length - 1)
        .split(",")
        .map(function (requiredArea) {
          return requiredArea.split("-").map(function (elm) {
            return Number(elm);
          });
        });

      this._timingBar = new HzTimingBar(
        x,
        y,
        hitArea,
        criticalArea,
  ...
  HzTimingBar.WIDTH = parameters["bar width"]
    ? Number(parameters["bar width"])
    : 500;
  ...
  HzTimingBar.prototype.initialize = function (
    x,
    y,
    hitArea,
    criticalArea,
    ...
    this._container.position.set(
      x - HzTimingBar.WIDTH / 2,
      y - HzTimingBar.HEIGHT / 2
    );

    // 枠
    var barFrameBmp = new Bitmap(HzTimingBar.WIDTH + 4, HzTimingBar.HEIGHT + 4);
    var framePath = roundedRectangle(
      barFrameBmp.context,
      2,
      2,
      HzTimingBar.WIDTH,
    ...
    for (var i = 0; i < this._requiredAreas.length; i++) {
      ...
      var reqWidth =
        (HzTimingBar.WIDTH * (requiredArea[1] - requiredArea[0])) /
        HzTimingBar.maxFrame;
      ...
      req.position.set(
        (HzTimingBar.WIDTH * requiredArea[0]) / HzTimingBar.maxFrame,
        0
      );
    ...
    // ヒットエリア
    var hitWidth =
      (HzTimingBar.WIDTH * (this._hitArea[1] - this._hitArea[0])) /
      HzTimingBar.maxFrame;
    ...
    hit.position.set(
      (HzTimingBar.WIDTH * this._hitArea[0]) / HzTimingBar.maxFrame,
      0
    );
    ...
    // クリティカルエリア
    var criticalWidth =
      (HzTimingBar.WIDTH * (this._criticalArea[1] - this._criticalArea[0])) /
      HzTimingBar.maxFrame;
    ...
    critical.position.set(
      (HzTimingBar.WIDTH * this._criticalArea[0]) / HzTimingBar.maxFrame,
      0
    );
  ...
  HzTimingBar.prototype.update = function () {
    ...
    if (this._frame >= 0) {
      ...
      this._cursor.x = 0;
      this._cursor.opacity =
        ((HzTimingBar.DELAY + this._frame) / HzTimingBar.DELAY) * 255;
    }
    // 時間経過でミス
    ...
    if (Input.isTriggered("ok") && this._frame >= 0) {
      ...
      if (
        this._criticalArea[0] <= this._frame &&
        this._frame < this._criticalArea[1]
      ) {
        ...
        this._hitArea[0] <= this._frame &&
        this._frame < this._hitArea[1]
      ) {
        if (this.allRequiredAreaHitted()) {
          result = 1;
          if (hitSe) {
