...
(function () {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (
      command === "オリジナルタイマー" ||
      command.toUpperCase() === "ORIGINALTIMER"
    ) {
      if ($gameTimer !== null) {
        if (
      ...
      if (CommndType == "表示" || CommndType.toUpperCase() == "DISPLAY") {
        opacity = 255;
        if (TimerSave == "YES")
          $gameTimer._fftfanttOriginalTimer_DisplayMode = "表示";
        return;
      }
      ...
      if (CommndType == "非表示" || CommndType.toUpperCase() == "HIDE") {
        opacity = 0;
        if (TimerSave == "YES")
          $gameTimer._fftfanttOriginalTimer_DisplayMode = "非表示";
        return;
      }
      ...
      if (
        CommndType == "設定時間操作" ||
        CommndType.toUpperCase() == "LIMITCHANGE"
      ) {
        if (!SetFlag) {
          if (!$gameTemp.isPlaytest())
            console.log("タイマーが設定されていません");
        ...
        var timer_tmp_array = args[2].match(
          /((\d+)(d|日))?((\d+)(h|時間))?((\d+)(m|分間?))?((\d+)(s|秒間?))?((\d+)(x))?((\d+)(c))?/
        );
        if (args[1] == "増加" || args[1] == "UP") {
          if (timer_tmp_array[2])
            TimerLimit =
              TimerLimit + parseInt(timer_tmp_array[2], 10) * 8640000;
          if (timer_tmp_array[5])
            TimerLimit = TimerLimit + parseInt(timer_tmp_array[5], 10) * 360000;
          if (timer_tmp_array[8])
            TimerLimit = TimerLimit + parseInt(timer_tmp_array[8], 10) * 6000;
          if (timer_tmp_array[11])
            TimerLimit = TimerLimit + parseInt(timer_tmp_array[11], 10) * 100;
          if (timer_tmp_array[14])
            TimerLimit = TimerLimit + parseInt(timer_tmp_array[14], 10) * 10;
          if (timer_tmp_array[17])
            TimerLimit = TimerLimit + parseInt(timer_tmp_array[17], 10);
      ...
      if (
        CommndType == "実時間操作" ||
        CommndType.toUpperCase() == "TIMECHANGE"
      ) {
        if (!SetFlag) {
          if (!$gameTemp.isPlaytest())
            console.log("タイマーが設定されていません");
        ...
        if (args[1] == "進行" || args[1].toUpperCase() == "UP") {
          if (timer_tmp_array[2])
            Count = Count + parseInt(timer_tmp_array[2], 10) * 8640000;
          if (timer_tmp_array[5])
            Count = Count + parseInt(timer_tmp_array[5], 10) * 360000;
          if (timer_tmp_array[8])
            Count = Count + parseInt(timer_tmp_array[8], 10) * 6000;
          if (timer_tmp_array[11])
            Count = Count + parseInt(timer_tmp_array[11], 10) * 100;
          if (timer_tmp_array[14])
            Count = Count + parseInt(timer_tmp_array[14], 10) * 10;
          if (timer_tmp_array[17])
            Count = Count + parseInt(timer_tmp_array[17], 10);
  ...
  function TimerSet(args) {
    ...
    TimerLimit = 0;
    if (timer_tmp_array[2])
      TimerLimit = TimerLimit + parseInt(timer_tmp_array[2], 10) * 8640000;
    if (timer_tmp_array[5])
      TimerLimit = TimerLimit + parseInt(timer_tmp_array[5], 10) * 360000;
    if (timer_tmp_array[8])
      TimerLimit = TimerLimit + parseInt(timer_tmp_array[8], 10) * 6000;
  ...
  function TimerGet(args) {
    ...
    if (
      args[1] == "日" ||
      args[1].toUpperCase() == "DAY" ||
      args[1].toUpperCase() == "D"
    ) {
    ...
    if (
      args[1] == "時" ||
      args[1].toUpperCase() == "HR" ||
      args[1].toUpperCase() == "H"
    ) {
    ...
    if (
      args[1] == "分" ||
      args[1].toUpperCase() == "MIN" ||
      args[1].toUpperCase() == "M"
    ) {
    ...
    if (
      args[1] == "秒" ||
      args[1].toUpperCase() == "SEC" ||
      args[1].toUpperCase() == "S"
    ) {
    ...
    if (
      args[1] == "コンマ秒" ||
      args[1].toUpperCase() == "HSEC" ||
      args[1].toUpperCase() == "XC"
    ) {
  ...
  Sprite_Picture.prototype.fftfanttOriginalTimer_UpdateShowText = function (
    bitmap
  ) {
    this.bitmap = new Bitmap(
      ShowText.length * fontsize + x,
      (fontsize + y) * 1.0
    );
